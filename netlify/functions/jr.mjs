...
export default async (request) => {
  ...
  if (path === '/jr/take') {
    ...
    try {
      ...
      const named = asked.map(safeKey);
      let round = 0;
      do {
        // Читаем по именам — и раз в секунду заглядываем в ящик целиком.
        //
        // Зачем заглядывать. Бот перечисляет тех, кто проходит проверку В
        // МОМЕНТ вопроса, а вопрос висит здесь до восьми секунд. Пришёл
        // новый человек, пока вопрос висит, — про него в этом вопросе не
        // спрошено, и его письмо пролежало бы до следующего захода бота. По
        // мобильному каналу через VPN заход стоит секунды: в журнале это
        // были «письмо пролежало в реле 8 с», а у человека — крутящаяся
        // кнопка после пройденной проверки. Здесь же перечисление стоит
        // миллисекунды: хранилище рядом с функцией, а не за каналом бота.
        //
        // Перечисление показывает только что записанное с задержкой, и
        // поэтому одному ему доверять нельзя — но как добавка к чтению по
        // имени оно может только помочь: раньше найдёт, а не позже.
        let keys = named;
        if (!asked.length || round % 4 === 0) {
          try {
            const all = (await mail.list()).blobs.map((b) => b.key);
            keys = asked.length ? [...new Set([...named, ...all])] : all;
          } catch (e) {
            keys = named;   // не вышло перечислить — идём по названным
          }
        }
        round += 1;
